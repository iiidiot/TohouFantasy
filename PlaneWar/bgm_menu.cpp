...
hgeGUIBGM::hgeGUIBGM(int _id, int _order, hgeFont *_fnt, HEFFECT _snd, float _x, float _y, float _delay, char *_title)
{
	float w;
...
void hgeGUIBGM::Update(float dt)
{
	if(timer2 != -1.0f)
	{
		timer2+=dt;
...
void hgeGUIBGM::Focus(bool bFocused)
{
	hgeColor tcolor;

	if(bFocused)
	{
		drawer.get_menu()->bgm_order = order;
		hge->Effect_Play(snd);
...
// the control's ID
bool hgeGUIBGM::MouseLButton(bool bDown)
{
	if(!bDown)
	{
		offset=4;
...
bool hgeGUIBGM::KeyClick(int key, int chr)
{
