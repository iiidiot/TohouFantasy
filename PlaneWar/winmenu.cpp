...
hgeGUIWinMenu::hgeGUIWinMenu(int _id, hgeFont *_fnt, HEFFECT _snd, float _x, float _y, float _delay, char *_title)
{
	float w;
...
void hgeGUIWinMenu::Update(float dt)
{
	if(timer2 != -1.0f)
	{
		timer2+=dt;
...
void hgeGUIWinMenu::Enter()
{
	hgeColor tcolor2;

	scolor2.SetHWColor(0x00FFFF22);
	tcolor2.SetHWColor(0xFFFFFF22);
...
void hgeGUIWinMenu::Leave()
{
	hgeColor tcolor2;

	scolor2.SetHWColor(0xFFFFFF22);
	tcolor2.SetHWColor(0x00FFFF22);
	dcolor2=tcolor2-scolor2;
...
void hgeGUIWinMenu::Focus(bool bFocused)
{
	hgeColor tcolor;
	...
	{
		hge->Effect_Play(snd);
		scolor.SetHWColor(0xFFFFFF22);
		tcolor.SetHWColor(0xFFFFFF22);
		sshadow.SetHWColor(0xFF220000);
		soffset=0;
		doffset=4;
	}
...
// the control's ID
bool hgeGUIWinMenu::MouseLButton(bool bDown)
{
	if(!bDown)
	{
		offset=4;
...
bool hgeGUIWinMenu::KeyClick(int key, int chr)
{
